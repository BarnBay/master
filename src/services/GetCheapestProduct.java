...
public class GetCheapestProduct extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ResultSet rs = db.executeSQL("SELECT * FROM TEST.CATEGORY WHERE IDCATEGORY =" + id);
		if(rs!=null){
			...
			try {
				if(rs.next()){
					...
					ResultSet rs2 = db.executeSQL("SELECT MIN(PRICE) FROM TEST.PRODUCT WHERE FK_CATEGORY=" + id);
					if(rs2!=null && rs2.next()){
						p.price = rs2.getInt(1);
					}
					
					ResultSet rs3 = db.executeSQL("SELECT * FROM TEST.PRODUCT WHERE FK_CATEGORY=" + id + "AND PRICE=" + p.price);
					int fk_farmer =0;
					...
					int fk_barnbay = 0;
					ResultSet rs4 = db.executeSQL("SELECT * FROM TEST.USERD WHERE IDUSER=" + fk_farmer);
					...
					ResultSet rs5 = db.executeSQL("SELECT * FROM TEST.BARNBAY WHERE IDBARNBAY=" + fk_barnbay);
					...
					ResultSet rs6 = db.executeSQL("SELECT * FROM TEST.PRODUCT WHERE FK_CATEGORY=" + id);
					ResultSet rs7;
